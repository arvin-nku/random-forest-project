test_that("greedy_cart works with valid input", {
  ...
  result <- greedy_cart(x = a, y = b, data = data_reg_li, type = "reg",
  ...
  expect_lte(nrow(dplyr::filter(result$tree, name == "leaf")), 10)
...
test_that("greedy_cart generates different trees with varying parameters", {
  ...
  tree1 <- greedy_cart(x = a, y = b, data = data_reg_li, type = "reg", num_leaf = 10)
  tree2 <- greedy_cart(x = a, y = b, data = data_reg_li, type = "reg", num_leaf = 20)
  ...
  tree3 <- greedy_cart(x = a, y = b, data = data_reg_li, type = "reg", depth = 3)
  tree4 <- greedy_cart(x = a, y = b, data = data_reg_li, type = "reg", depth = 5)
...
test_that("greedy_cart creates valid tree structure", {
  ...
  expect_equal(nrow(dplyr::filter(result$tree, name == "leaf")), 2)
